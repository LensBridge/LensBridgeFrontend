...
export function describeContent(content) {
  ...
  if (content.staleDays > 0) {
    return `${range}: ran out ${content.staleDays} day${content.staleDays === 1 ? '' : 's'} ago`;
  }
  const left = content.daysRemaining;
...
export function boardIssues(board) {
  ...
  if (c?.staleDays > 0) issues.push({ level: 'danger', label: `Content ran out ${c.staleDays}d ago` });
  else if (c && c.daysRemaining <= CONTENT_LOW_DAYS) issues.push({ level: 'warning', label: `Content ends in ${c.daysRemaining}d` });
